...
async function main() {
  ...
  console.log("liquidityUserBorrowData:", data.liquidityUserBorrowData);
  console.log("liquidityUserSupplyData:", data.liquidityUserSupplyData);
  console.log("totalSupplyAndBorrow:", data.totalSupplyAndBorrow);

  const legs = await detectSmartLegs(publicClient, VAULT);
  if (legs.debtDex) {
    const reserves = await loadDexDebtReserves(publicClient, legs.debtDex);
    console.log("real DEX pool debt reserves:", reserves);
  }
}
