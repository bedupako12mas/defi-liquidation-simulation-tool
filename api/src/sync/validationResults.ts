...
import { validateFluidLiquidation, resolveFluidOverrideTarget } from "../validation/fluidValidator.js";
import { redactError } from "../rpc/redact.js";
import type { ValidationResultsTable } from "../db/types.js";
import type { Insertable } from "kysely";
import type { AssetShockConfig } from "../engine/shockModel.js";
import type { PriceVector } from "../engine/types.js";

// Lives under src/, not scripts/, specifically so it gets compiled into dist/sync/validationResults.js
// by the normal `npm run build` - scripts/ is dev-only and was never copied into the Docker image
// (same real gap src/db/migrate.ts's own top comment describes and fixes for migrations). This is
// what k8s/base/sync-validation-job.yaml's `node dist/sync/validationResults.js` actually executes.
//
...
export async function runSyncValidationResults(): Promise<boolean> {
  try {
...
// Self-executing when run directly (`node dist/sync/validationResults.js` in the real
// container/Job, or `tsx src/sync/validationResults.ts` / via scripts/sync-validation-results.ts
// locally) - NOT when imported as a library elsewhere, same convention as src/db/migrate.ts.
