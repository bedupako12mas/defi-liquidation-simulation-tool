...
import { applyShock, SHOCK_PRESETS } from "../engine/shockModel.js";
import { healthFactor } from "../engine/healthFactor.js";
import { validateAaveLiquidation } from "../validation/aaveValidator.js";
import { startAnvilFork, type AnvilFork } from "../fork/anvilFork.js";
import { buildFixedReturnBytecode, buildFixedTupleReturnBytecode } from "../validation/stateOverride.js";
import { probeTokenSlots } from "../validation/slotProbe.js";
import { redactError } from "../rpc/redact.js";
import { parseAbi, encodeFunctionData, decodeFunctionResult, decodeErrorResult, decodeEventLog, keccak256, encodeAbiParameters, numberToHex, getAddress, type PublicClient } from "viem";
import { FLUID_VAULT_RESOLVER } from "../loaders/fluidAddresses.js";
import { resolveSmartLegOracle } from "../validation/fluidT2OracleValidator.js";
...
import type { PriceVector, Position } from "../engine/types.js";
import type { AaveReserveConfig } from "../loaders/aaveReserveConfig.js";
import type { Insertable } from "kysely";
import type { ChainedLiquidationResultsTable } from "../db/types.js";
...
import { resolveFluidOverrideTarget, validateFluidLiquidation, estimateFluidLiquidationGas, extractRevertData } from "../validation/fluidValidator.js";
import type { FluidVaultConfig } from "../loaders/fluidVaultConfig.js";
import { AAVE_V4_SPOKES, type AaveV4SpokeName } from "../loaders/aaveV4Addresses.js";
import { validateAaveV4Liquidation } from "../validation/aaveV4Validator.js";
...
async function dryRunT2LiquidateOnClient(
  // PublicClient (viem's generic type), not `typeof publicClient` - this is called with
  // both the module-level publicClient AND fork.publicClient (AnvilFork's own PublicClient-
  // typed field), and `typeof publicClient`'s narrower literal chain type isn't assignable
  // from the generic one AnvilFork declares. Real, pre-existing type gap never caught before
  // today - scripts/ was excluded from tsconfig.json's `include`, so `tsc` never checked
  // this file at all until it moved to src/.
  client: PublicClient,
  vault: `0x${string}`,
  legs: { overrideAddress: `0x${string}`; currentRate: bigint }[],
  magnitudePct: number,
...
export async function runSyncChainedLiquidation(): Promise<boolean> {
  try {
...
// Self-executing when run directly, same convention as src/sync/validationResults.ts / src/db/migrate.ts.
