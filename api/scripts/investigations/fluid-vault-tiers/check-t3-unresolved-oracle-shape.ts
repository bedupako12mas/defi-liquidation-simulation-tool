...
const CANDIDATES = [
  { name: "chainlinkOracleData()", fn: "chainlinkOracleData", sig: "function chainlinkOracleData() view returns (uint256,address,bool,uint256,address,bool,uint256,address,bool,uint256)" },
  { name: "infoName()", fn: "infoName", sig: "function infoName() view returns (string)" },
  { name: "dexSmartDebtSharesRates()", fn: "dexSmartDebtSharesRates", sig: "function dexSmartDebtSharesRates() view returns (uint256,uint256)" },
  { name: "RESERVES_PEG_BUFFER_PERCENT()", fn: "RESERVES_PEG_BUFFER_PERCENT", sig: "function RESERVES_PEG_BUFFER_PERCENT() view returns (uint256)" },
  { name: "getDexColDebtOracleData()", fn: "getDexColDebtOracleData", sig: "function getDexColDebtOracleData() view returns (address,bool)" },
...
async function main() {
  ...
  for (const c of CANDIDATES) {
    try {
      ...
      const result = await publicClient.readContract({ address: oracle, abi, functionName: c.fn as any });
