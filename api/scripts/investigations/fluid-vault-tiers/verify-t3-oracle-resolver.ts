...
async function main() {
  ...
  for (const vault of t3Vaults) {
    ...
    console.log(`${vault} [${resolution.shape}] overridable=${resolution.overridable.map(o=>o.leg).join(",")||"NONE"}`);
    if (resolution.overridable.length > 0) resolvable++; else unresolvable++;
  }
  console.log(`\nTotal T3: ${t3Vaults.length}, resolvable: ${resolvable}, unresolvable: ${unresolvable}, zero-oracle: ${zeroOracle}`);
}
