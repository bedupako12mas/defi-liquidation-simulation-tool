...
const VAULT = "0xB170B94BeFe21098966aa9905Da6a2F569463A21" as const;
...
async function main() {
  ...
  const infoNameAbi = parseAbi(["function infoName() view returns (string)"]);
  const infoName = await publicClient.readContract({ address: oracle, abi: infoNameAbi, functionName: "infoName" }).catch((e) => `FAIL: ${e}`);
